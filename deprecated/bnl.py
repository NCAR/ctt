...
def gzip_log():
    ...
    yesterday_log = '/ssg/logs/bnl/%s.%s.log' % (host, yesterday)
    if os.path.isfile(yesterday_log):
	with open(yesterday_log, 'rb') as f_in, gzip.open('%s.gz' % (yesterday_log), 'wb') as f_out:
	    shutil.copyfileobj(f_in, f_out)

        os.unlink(yesterday_log)


def nodesort(t):
    def node_trans(t):
        return t[0].replace('r','').replace("i",".").replace("n",".")
    return map(int,node_trans(t).split('.'))
...
def log_history(history_entry):
	logging.basicConfig(filename=logfile, format='%(asctime)s %(message)s', level=logging.INFO)
	logging.info(history_entry)

def get_node_count():
    """ Get the number of nodes already in the bnl. This is for max_ev_tickets """
    with open(CLUSH_BAD_GROUPS, 'r') as f:
        all_nodes = yaml.load(f)
        all_nodes = (all_nodes['bad']['nodes'])
...
def add_nodes(nodes, comment, new_state = 'suspect-pending', skip_ev = False): 
	...
	if not node in STATE['nodes']:
	    ...
	    vlog(3, 'Added %s' % (node))
	    log_history('Added %s; comment: %s' % (node, comment)) 

        if len(STATE['nodes'][node]['extraview']) < 1 and not skip_ev:
	    if int(node_count) >= int(max_ev_tickets) and 'auto' in argv[1:]:
		break
	    else:
		ev_id = EV.create( \
			'ssgev', \
		...
		if ev_id:
		    ...
		    vlog(3, 'opened EV#%s for %s' % (ev_id, node))
		    log_history('Opened EV#%s for %s; comment: %s' % (ev_id, node, comment))  
...
def del_nodes(nodes, comment):
	...
	log_history('Releasing node %s; comment: %s' % (node, comment))

	for ev_id in STATE['nodes'][node]['extraview']:
	    EV.close(ev_id, 'Bad Node Comment:\n%s' % comment)
	    vlog(3, 'closed Extraview Ticket %s for %s' % (ev_id, node))
            log_history('Closed extraview ticket %s for %s' % (ev_id, node))

	del STATE['nodes'][node]
	vlog(3, 'released node %s; comment: %s' % (node, comment))
	log_history('Released node %s; comment: %s' % (node, comment))
...
def list_state(nodes):
    ...
    print '{:<14}{:<20}{:<14}{:<40}{:<20}'.format('Node','state','Extraview','comment', 'scheduler comment')
    for node,state in sorted(STATE['nodes'].iteritems(),key=nodesort):
	if len(nodelist) == 0 or node in nodelist:
	    print '{:<14}{:<20}{:<14}{:<40}{:<20}'.format(node,state['state'], ','.join(state['extraview']),state['comment'], state['scheduler_comment'])
...
def comment_nodes(nodes, comment):
	...
	for ev_id in STATE['nodes'][node]['extraview']:
	    ...
	    vlog(3, '%s EV#%s comment: %s' % (node, ev_id, comment))
            log_history('%s EV#%s comment: %s' % (node, ev_id, comment))
	
	log_history('Updated comment for %s: %s' % (node, comment))		
...
def attach_nodes(nodes, ev_ids):
	...
	for ev_id in ev_ids:
		...
		vlog(3, 'node %s add extraview %s' % (node, ev_id))
                log_history('%s add extraview %s' % (node, ev_id)) 
...
def detach_nodes(nodes, ev_ids):
	...
	for ev_id in ev_ids:
		...
		vlog(3, 'node %s remove extraview %s' % (node, ev_id))
                log_history('Remove extraview %s for %s' % (ev_id, node)) 
...
def mark_hardware(nodes, comment):
	...
	log_history('%s marked as bad hardware; comment: %s' % (node, comment)) 
...
def mark_casg_ready(node):
    ...
    vlog(3, 'shutting down %s for casg' % (node))
    log_history('Shutting down %s for casg' % (node))  
    ...
    for ev_id in STATE['nodes'][node]['extraview']:
	vlog(3, 'sending EV#%s for %s to casg' % (ev_id, node))
	log_history('Sending EV#%s for %s to casg' % (ev_id, node))	
...
def mark_sibling_ready(node):
    ...
    log_history('Shutting down sibling %s' % (node))   
...
def mark_hardware_ready(node):
    ...
    vlog(3, 'shutting down %s for hardware' % (node))
    log_history('Shutting down %s for hardware' % (node))
...
def run_auto():
	...
	if known_bad and not 'offline' in states:
	    vlog(2, 'bad node %s was not offline in pbs' % (node))
            log_history('Bad node %s was not offline in pbs' % (node))
	...
	if node in jobless and 'offline' in states and known_bad:
		...
		if node in sibst['siblings'] and not sib in jobless:
		    ...
		    vlog(5, '%s has sibling %s with job' % (node, sib))
                    log_history('%s has sibling %s with job' % (node, sib))
		...
		vlog(4, 'bad node %s skipped due to sibling jobs' % (node))
                log_history('Bad node %s skipped due to sibling jobs' % (node))
	...
	if node in STATE['nodes']:
		...
		for ev_id in STATE['nodes'][node]['extraview']:
		    ...
		    log_history('%s EV#%s; comment: pbs state change: %s' % (node, ev_id, scmt)) 
		STATE['nodes'][node]['scheduler_comment'] = scmt
		log_history('Scheduler comment for %s: %s' % (node, scmt))  #JON
 
    vlog(4, 'checking ipmi power status of %s nodes' % (len(check_nodes)))
    log_history('Checking ipmi power status of %s nodes' % (len(check_nodes)))
	...
	vlog(2, 'unable to call ipmi power status for %s nodes' % (len(check_nodes)))
        log_history('Unable to call ipmi power status for %s nodes' % (len(check_nodes)))
    else:
...
def release_pending_node(node):
    ...
    state = STATE['nodes'][node]['state']
    #vlog(4, 'changing %s from pending state to %s state' % (node, state))
    #log_history('Changing %s from pending state to %s state' % (node, state))
...
max_ev_tickets = 0
""" Maximum number of ev tickets bnl can have at a given time """

""" get hostname for history logging """
host = socket.gethostname()

""" get date for history logging """
date = time.strftime("%Y-%m-%d") 

""" set logfile for history logging """
logfile = '/ssg/logs/bnl/%s.%s.log' % (host, date)
